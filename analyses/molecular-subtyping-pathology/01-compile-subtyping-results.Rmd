...
```{r}
data_dir <- file.path("..", "..", "data","release_003_008")
analyses_dir <- ".."
...
hgg_results_df <- hgg_results_df %>%
  dplyr::mutate(
    integrated_diagnosis = case_when(
      grepl("DMG, H3 K28",molecular_subtype)~"Diffuse midline glioma",
      grepl("HGG, H3 G35",molecular_subtype)~"High-grade glioma",
      TRUE ~ "High-grade glioma"))
